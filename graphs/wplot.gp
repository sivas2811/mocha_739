set title sprintf("Write Scalability")
set xrange [0: 3000]
set yrange [0:350]
dirname_r = sprintf(".")
dirname_w = sprintf(".")
set ylabel "Completion Time (seconds)"
set xlabel "Total number of parallel write requests" offset character 1
set terminal postscript enhanced eps color "Helvetica" 30
set output sprintf("%s/write_scalability.eps", dirname_w)
...
plot sprintf("%s/write_scal.dat", dirname_r) using 2:3 title "5000 requests" w lines linecolor rgb sample_color,\
